...
\usepackage[colorinlistoftodos]{todonotes} % To add a note use /todo
\usepackage{soulutf8} %To markup text use \hl
\sethlcolor{yellow}
...
\usepackage{glossaries} % for editor autocomplete
...
\author{João \textsc{Sousa}} % Your name, this is used in the title page, print it elsewhere with \authorname

\subjectarea{Engenharia de Software} % Specialisation area, used in the title page, print it elsewhere with \areaname

\advisor{Paulo Gandra \textsc{Sousa}} % Your advisors's (academic mentor) name, this is used in the title page, print it elsewhere with \advname

\supervisor{João \textsc{Peixoto}} % Your supervisor's (company, hosting institution mentor) name, this is used in the title page, print it elsewhere with \supname
...
\keywords{Modelo de Linguagem de Grande Escala, Inteligência Artificial, Processamento da Linguagem Natural, Incidente, Mitigação, Sugestão} % Please define up to 6 keywords that better describe your work, print it elsewhere with \keywordnames
...
\input{ch1/chapter1}
\input{ch2/chapter2_0}
\input{ch2/chapter2}
%\input{ch3/chapter3} Futuros Desenvolvimentos de PREPD --> Passou para chapter4
\input{ch4/chapter4}
%\input{ch5/chapter5} Juntamos à Analise
\input{ch6/chapter6}
\input{ch7/chapter7}
\input{ch8/chapter8}
