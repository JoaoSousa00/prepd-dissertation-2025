...
\label{chap:Chapter4} % For referencing the chapter elsewhere, use \ref{chap:Chapter4} 
	...
	Neste capítulo, são abordados os próximos passos para o desenvolvimento da solução a ser proposta. Assim, é definida a metodologia a ser seguida na elaboração do projeto, é apresentado um desenho da solução onde, são identificados os sistemas externas a ser utilizados. \hl{São, ainda, identificados os pressupostos e restrições inerentes à solução proposta neste documento.} Também é abordada a estrutura e origem dos dados pertencentes ao \textit{dataset} assim como a apresentação dos requisitos funcionais e não funcionais que integram a engenharia de requisitos.
...
%----------------------------------------------------------------------------------------
	\subsection{Requisitos Funcionais}
		...
		\begin{itemize}
			\item \textit{Must have} - Essencial para o funcionamento da solução;
			\item \textit{Should have} - Importante mas não impede o funcionamento da solução caso não seja implementado;
			\item \textit{Could have} - Pode ser incluído caso existam recursos e tempo disponíveis;
			\item \textit{Won't have} - Não está nos planos do projeto.
		\end{itemize}
		...
		\begin{table}[H]
			\centering
			\begin{tabular}{p{2.1cm}p{9cm}p{1.8cm}}
				\hline
	...
	\subsection{Requisitos Não Funcionais}
		...
		\begin{table}[H]
			...
			\begin{tabular}{p{2.2cm}p{11cm}}
				...
				\hline
				\textit{RNF-01} & O sistema deve ser capaz de continuar a processar pedidos desde que os serviços externos necessários estejam disponíveis \\
				\hline
				\textit{RNF-02} & O sistema não deve expor informação confidencial dos incidentes a utilizadores ou serviços que não tenham autorização para a consultar \\
				\hline
				\textit{RNF-03} & O sistema deve estar organizado de forma a permitir adicionar, substituir ou remover serviços externos sem impacto no restante serviço \\
				\hline
				\textit{RNF-04} & O sistema deve limitar a quantidade de informação enviada para o \gls{LLM}, de forma a manter o custo de cada análise dentro de valores aceitáveis \\
				\hline
				\textit{RNF-05} & O sistema deve permitir identificar os incidentes utilizados como contexto para cada análise \\
				\hline
				\textit{RNF-06} & As respostas geradas devem apresentar um nível de qualidade adequado para apoiar a análise e resolução de incidentes \\
				\hline
			\end{tabular}
