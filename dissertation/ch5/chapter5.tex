...
\label{chap:Chapter5} % For referencing the chapter elsewhere, use \ref{chap:Chapter5} 
	...
	\todo{Juntar este capitulo com a Analise}
...
\section{Diagrama de Sequência}
	...
	\todo{Rever e adicionar mais detalhes}
% Caso do Fallback vvvv
%	Apresenta-se apenas um diagrama de sequência que, representa um dos cenários de recuperação de contexto, especialmente útil para a analise de incidentes manuais. O diagrama descreve o fluxo seguido quando a pesquisa por incidentes históricos com o mesmo título não devolve resultados. Nesse caso, o sistema realiza uma segunda pesquisa, obtendo os incidentes (fechados e ativos) mais recentes da equipa em questão, de forma a encontrar outros problemas que possam estar relacionados com o incidente em análise. O diagrama na imagem \ref{fig:sequenceDiagram} permite visualizar a interação entre os principais componentes e compreender o comportamento da solução neste cenário.
	...
	Apresenta-se apenas um diagrama de sequência que representa um fluxo bem sucedido de recuperação e enriquecimento de contexto utilizado na análise de um incidente. O diagrama começa com a obtenção dos detalhes do incidente através do serviço de ITSM, sendo posteriormente utilizada uma chamada ao \gls{LLM} para identificar incidentes relacionados e gerar uma pesquisa para a documentação do Confluence. Com base nessa pesquisa, o sistema obtém as páginas relacionadas com o serviço e o respetivo conteúdo.
	
	De seguida, o conteúdo das páginas é enviado ao \gls{LLM}, que identifica a documentação mais relevante para o incidente e resume a informação encontrada. Após esta etapa, são obtidos os incidentes relacionados e toda a informação recolhida é utilizada numa chamada final ao \gls{LLM}, responsável por enriquecer a informação do incidente. O diagrama na Figura \ref{fig:sequenceDiagram} permite visualizar a interação entre os principais componentes e compreender as diferentes etapas utilizadas para obter e combinar o contexto necessário para a análise do incidente.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth,keepaspectratio]{img/diagramaSequencia_v2}
