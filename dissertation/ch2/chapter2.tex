...
\section{Revisão de Literatura}
	...
	\subsection{Apoio à Contextualização de Incidentes com \textit{LLMs}}
		...
		Esta falta de contexto na fase inicial, impele os \gls{OCEs} a iniciar uma investigação com base em informação limitada, o que faz com que os elementos de prevenção \hl{iniciem} várias tentativas para entender o impacto real do incidente e os serviços que são afetados. Em equipas com aplicações de grande escala, onde múltiplos incidentes podem ocorrer em simultâneo, esta primeira impressão incompleta do incidente pode contribuir para atrasos na resposta e aumentar a pressão sobre os \gls{OCEs}.
...
\section{Conclusão}
	...
	Com este capítulo, foi possível verificar, com base nos trabalhados analisados, que a gestão de incidentes em sistemas de grande escala é ainda muito afetada pela fragmentação da informação relacionada com as falhas que causam os incidentes, pela recorrência e duplicação de \textit{tickets} em curtos intervalos de tempo e pela dependência da experiência dos \gls{OCEs}. Assim, a literatura, mostrou que o uso de \gls{IA}, como \gls{LLM}\textit{s} ou \gls{GNNs}, é benéfico no sentido em que facilita a contextualização inicial dos incidentes aos membros da equipa de suporte, e ajuda na mitigação das falhas ao fornecer detalhes e sugestões de resolução com base em registos de incidentes anteriores.
